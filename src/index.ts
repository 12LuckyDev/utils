 ...
 */
export const editAt = <T>(array: Array<T>, value: T, index: number) => array.map((v, i) => (i === index ? value : v));
 ...
 */
export const removeAt = <T>(array: Array<T>, index: number) => array.filter((v, i) => i !== index);
...
) => array.filter((v) => !values.includes(v[key]));
 ...
 */
export const nMap = <T>(n: number, callback: (i: number) => T) => [...new Array(n).keys()].map(callback);
