...
test('TESTING to2D func with no params', () => {
  ...
  expect(outputArray1[0]).toEqual([1, 2, 3]);
  expect(outputArray1[2]).toEqual([7, 8, 9]);
});
...
test('TESTING to2D func with set size', () => {
  ...
  expect(outputArray1[0]).toEqual([1, 2, 3, 4, 5]);
  expect(outputArray1[1]).toEqual([6, 7, 8, 9]);

  expect(outputArray2.length).toEqual(5);
  expect(outputArray2[0]).toEqual([1, 2]);
  expect(outputArray2[4]).toEqual([9]);
});
...
test('TESTING to2D func with cols direction', () => {
  ...
  expect(outputArray1[2]).toEqual([3, 6, 9]);
});
...
test('TESTING to2D func with cols direction and set size', () => {
  ...
  expect(outputArray1[2]).toEqual([3, 7]);
